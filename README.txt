...
Example of the definition of various objects:
  ...
  task_context "ExampleTask" do
    ...
    # Create Orocos::Generation::OutputPort and Orocos::Generation::InputPort
    # objects to describe orocos ports
    output_port('dout', 'double').
      doc 'an output port of type double'
    input_port('din', '/std/string', 'r').
      doc 'an input port of type std::string'
  end
  ...
  periodic in the deployment:
    ...
    static_deployment do
      my_task = task("MyTaskContext").
        periodic(0.01) # 10ms period
    end

* a port-driven task is trigerred each time one of the specified input ports is
  updated.
  ...
  It is additionally possible to have tasks that are both port-driven and
  periodic (for instance), in which case they get triggered when either the port
  is updated or when the period is reached.
  ...
  Moreover, the generated task context class defines a

    void updateHook(std::vector<PortInterface*> const& ports);

  instead of defining an updateHook(void) method. The given vector lists the
  ports that have triggered the update.

* a IO-driven task is triggered when new data is made available on the
  underlying IO file descriptor. As for port-driven tasks, IO-driven
  ...
  To be IO-driven, no activity should be defined in the deployment section.
  Nonetheless, it is possible to specify a different triggering mechanism, at
  the cost of not being IO-driven anymore. Moreover, the generated task context
  class defines a

    int getFileDescriptor() const;

  method, which must be implemented to return the file descriptor that should be
  listened to.
...
* for each task object named <tt>object</tt> (attribute, port, ...), there is
  ...
  * ports defined by TaskContext#input_port and TaskContext#output_port are
    mapped to RTT::InputPort and RTT::OutputPort respectively[available starting at RTT2]
...
== Component examples
There is quite a few component examples in the test suite. See
<tt>test/data/modules</tt>. These examples are quite heavily commented, so they
can serve as a starting point to build your own module.
